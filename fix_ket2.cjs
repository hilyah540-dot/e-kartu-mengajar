...
const mengajarBlockOld = `        const ket = row.Keterangan || '-';
        if (ulya > 0 || wus > 0 || td > 0) {`;

const mengajarBlockNew = `        const ket = (isMengajar ? row.Keterangan : row.Keterangan_Pengganti) || '-';
        if (ulya > 0 || wus > 0 || td > 0) {`;

code = code.replace(mengajarBlockOld, mengajarBlockNew);
